...
class BrowseActivity : ListActivity() {
    ...
    private fun getData(): List<Map<String, Any>> {
        ...
        myData.add(
            mapOf(
                "title" to "ViewPager+Fragment+RESUME_ONLY->lifecycle",
                "intent" to activityToIntent(ResumeOnlyFragmentActivity::class.java.name)
            )
        )
        myData.add(
            mapOf(
                "title" to "ViewPager+Fragment+SET_USER->lifecycle",
                "intent" to activityToIntent(SetUserFragmentActivity::class.java.name)
            )
        )
        myData.add(
            mapOf(
                "title" to "ViewPager2+Fragment+FragmentStateAdapter->lifecycle",
                "intent" to activityToIntent(ViewPager2FragmentActivity::class.java.name)
            )
        )
